...
static MwLLGL MwLLGLCreateImpl(MwLL handle) {
	...
	r->x11.gl     = r->x11.glXCreateContext(handle->x11.display, r->x11.visual, NULL, GL_TRUE);
	return r;
}
...
static void MwLLGLMakeCurrentImpl(MwLL ll, MwLLGL gl) {
	gl->x11.glXMakeCurrent(ll->x11.display, ll->x11.window, gl->x11.gl);
}

static void MwLLGLSwapBufferImpl(MwLL ll, MwLLGL gl) {
	gl->x11.glXSwapBuffers(ll->x11.display, ll->x11.window);
}

static void* MwLLGLGetProcAddressImpl(MwLL ll, MwLLGL gl, const char* name) {
	return gl->x11.glXGetProcAddress((const GLubyte*)name);
}
#endif
