...
+ (MilskoCocoa*)newWithParent:(MwLL)parent
	...
	[c->window setDelegate:[[MilskoCocoaWindowDelegate alloc]
...
- (void)polygonWithPoints:(MwPoint*)points
	...
	if(ctx) {
		...
		NSColor*      nscolor = [NSColor colorWithCalibratedRed:color->common.red / 255.
...
- (void)eventProcess:(NSEvent*)ev {
	...
	if([[[win contentView] subviews] count] == 0) {
		...
		h = [((MilskoFakePointer*)[[[win contentView] subviews] objectAtIndex:0]) pointer];
	}
...
- (void)setClipboard:(const char*)text {
	...
	// NSAutoreleasePool* pool	      = [[NSAutoreleasePool alloc] init];
	// NSPasteboard*	   pasteboard = [NSPasteboard generalPasteboard];
	// [pasteboard declareTypes:[NSArray arrayWithObjects:NSPasteboardTypeString] owner:nil];
